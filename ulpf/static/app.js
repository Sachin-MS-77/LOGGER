"use strict";
const savedTheme = localStorage.getItem("aegis-interface-theme") || "daylight";
document.documentElement.dataset.theme = ["daylight", "midnight", "crimson"].includes(savedTheme) ? savedTheme : "daylight";
...
document.querySelector("#theme-select").addEventListener("change", (event) => {
  ...
  localStorage.setItem("aegis-interface-theme", event.target.value);
});
...
const S = {
  page: "overview",
  token: sessionStorage.getItem("aegis-token") || "",
  stats: null,
  system: null,
  candidateId: null,
  candidate: null,
...
async function overview() {
  const s = S.stats;
  const recent = await api("/events?limit=5");
  const pending = ["unparsed","drifted","failed","partial"].reduce((n,k)=>n+(s.statuses[k]||0),0);
  const live = s.modes.filter(x=>x.mode==="live").reduce((n,x)=>n+x.count,0);
  const delivered = s.outbox.find(x=>x.status==="delivered")?.count || 0;
  const listenerCount = Object.values(S.system.listeners).filter(x=>x.listening).length;
  const node=(ic,title,sub,cls="",action="sources")=>`<button class="flow-node ${cls}" data-action="nav" data-page="${action}">${icon(ic)}<div><strong>${title}</strong><small>${sub}</small></div></button>`;
  return `<section class="command-hero"><div class="hero-copy"><div class="eyebrow">AEGIS / PERIMETER INTELLIGENCE</div><h1>Every log. A clearer picture.</h1><p>Your security telemetry, connected and understood.<br>Capture the original. Trace every transformation.</p></div><div class="hero-actions">${button(S.system.demo.running?"Replay running…":"Launch demo","demo","play","primary",S.system.demo.running?"disabled":"")}${button("Connect a device","add-source","plus")}</div></section>`+
    `<div class="status-strip"><span class="pulse">COLLECTOR ${listenerCount?"ONLINE":"OFFLINE"}</span><span><b>${listenerCount}</b> active transports</span><span><b>${s.plugins}</b> deployed parsers</span><span class="right">${S.system.demo.running?`REPLAY ${S.system.demo.processed} / ${S.system.demo.total}`:`${fmt(live)} live · ${fmt(s.total-live)} replay / lab events`}</span></div>`+
    `<div class="metrics">${metric("TOTAL EVENTS",fmt(s.total),"",`${bytes(s.raw_bytes)} original bytes preserved`,"database")}${metric("NORMALIZATION",s.coverage,"%",`${fmt(s.normalized)} mapped · partials included`,"activity")}${metric("EVIDENCE SEALED",pct(s.sealed,s.total),"%",`${fmt(s.sealed)} events in Merkle batches`,"shield")}${metric("REVIEW QUEUE",fmt(s.pending_review),"",`${fmt(pending)} events need attention`,"lab","attention")}</div>`+
    `<div class="operations-grid">${panel("Telemetry pipeline",`<div class="flow-map"><div class="flow-sources">${node("shield","Firewalls","Syslog · CEF · LEEF")}${node("network","Routers","TCP · UDP · TLS")}${node("activity","IDS / applications","JSON · XML · CSV")}</div><div class="flow-wire"></div>${node("shield","AEGIS",`${fmt(s.total)} captured · ${s.formats.length} formats`,"core","events")}<div class="flow-wire"></div><div class="flow-output">${node("database","Evidence vault",`${fmt(s.sealed)} sealed events`,"","evidence")}${node("graph","Analytics / SIEM",`${fmt(delivered)} HTTP deliveries`,"","system")}</div></div><div class="flow-legend"><span><b>01</b> CAPTURE</span><span><b>02</b> PRESERVE</span><span><b>03</b> NORMALIZE</span><span><b>04</b> VERIFY</span></div>`,badge("LIVE STATE","green"))}${panel("Operator priorities",`<div class="signal-stack"><div class="signal-card">${icon("lab")}<div><h3>Unfamiliar formats</h3><p>Review mappings before deployment.</p><button class="link-button" data-action="nav" data-page="lab">Open parser lab →</button></div><span class="signal-number">${s.pending_review}</span></div><div class="signal-card">${icon("activity")}<div><h3>Detection signals</h3><p>Rule matches across observed events.</p><button class="link-button" data-action="nav" data-page="investigate">Investigate signals →</button></div><span class="signal-number">${s.alerts}</span></div><div class="signal-card">${icon("sources")}<div><h3>Source coverage</h3><p>${s.source_count} observed sources · labeled traffic modes</p><button class="link-button" data-action="nav" data-page="sources">Manage devices →</button></div></div></div>`,badge("TRIAGE","amber"))}</div>`+
    `<div class="overview-secondary">${panel("Ingestion activity",chart(),`<span class="range-button">${icon("time")}30 MIN / UTC</span>`)}${panel("Chain of custody",`<div class="integrity-art">${icon("shield","shield-icon")}<div><strong>${fmt(s.sealed)}</strong><p>events with sealed provenance</p></div></div><div class="integrity-lines"><div class="integrity-line"><span>Witness quorum</span><strong>2 of 3 · local</strong></div><div class="progress mt"><span style="width:${pct(s.sealed,s.total)}%"></span></div></div><div class="integrity-bottom"><span>SHA-256 / Merkle / Ed25519</span><button class="link-button" data-action="nav" data-page="evidence">Verify →</button></div>`,badge("VERIFIABLE","green"))}</div>`+
    panel("Latest telemetry",eventTable(recent.items,{compact:true})+`<div class="table-footer"><span>Latest ${recent.items.length} of ${fmt(recent.total)} · all traffic modes</span><button class="link-button" data-action="nav" data-page="events">Open event stream →</button></div>`,badge("AUTO REFRESH / 4s","outline"));
}
...
async function labPage() {
  ...
  let detail = empty(
    ...
    "When a new format arrives, AEGIS preserves it and creates a review candidate. Run the replay to try onboarding.",
...
async function evidencePage() {
  ...
  return (
    ...
    `<div class="section-gap">${panel("Inspect original evidence", eventTable(recent.items, { compact: true }), '<span class="tiny muted">Open an event → Verify evidence</span>')}</div>`
  );
}
function graphSvg(data) {
  ...
  const all = data.nodes.slice(0, 15);
  if (!all.length)
    return empty(
      "No mapped relationships",
      "Normalize events with source and destination addresses to explore their relationships.",
    );
  const W = 650,
    H = 270,
    cx = 325,
  ...
  all.forEach((n, i) => {
    pos[n.id] =
      all.length === 1
        ? [cx, cy]
        : [
            cx + Math.cos((i / all.length) * 2 * Math.PI) * 235,
            cy + Math.sin((i / all.length) * 2 * Math.PI) * 85,
          ];
  });
  return `<div class="graph"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="Observed network address relationships">${Array.from(
    counts.entries(),
  )
    .filter(([key]) => key.split("|").every((k) => pos[k]))
    .map(([key, n]) => {
      const [a, b] = key.split("|");
      return `<line x1="${pos[a][0]}" y1="${pos[a][1]}" x2="${pos[b][0]}" y2="${pos[b][1]}" stroke="#bdcfdc" stroke-width="${Math.min(3, 1 + n / 20)}"><title>${esc(a)} → ${esc(b)}: ${n} events</title></line>`;
    })
    .join(
      "",
    )}${all.map((n) => `<g class="node" data-action="graph-ip" data-ip="${esc(n.id)}" tabindex="0" role="button" aria-label="Investigate ${esc(n.id)}"><circle cx="${pos[n.id][0]}" cy="${pos[n.id][1]}" r="${n.id === S.ip ? 17 : 12}" fill="${n.id === S.ip ? "#183b4b" : "#e4f1eb"}" stroke="#83b7a0" stroke-width="1.5"/><circle cx="${pos[n.id][0]}" cy="${pos[n.id][1]}" r="3" fill="${n.id === S.ip ? "#b4f2d9" : "#519479"}"/><text x="${pos[n.id][0]}" y="${pos[n.id][1] + 30}" fill="#5b7287" text-anchor="middle">${esc(n.id)}</text></g>`).join("")}</svg></div>`;
}
async function investigatePage() {
  const [g, alerts, cases] = await Promise.all([
    api("/graph?ip=" + encodeURIComponent(S.ip)),
    api("/alerts"),
    api("/cases"),
  ]);
  S.graph = g;
  return (
    ...
    `<div class="filters panel mb"><input id="graph-search" class="search-input" placeholder="Focus on an IP address, or leave blank for all" aria-label="Investigation IP" value="${esc(S.ip)}">${button("Investigate", "investigate-search", "search")}${S.ip ? button("Clear", "clear-ip", "", "small") : ""}<span class="tiny muted">${g.behavior.observing_sources} observing sources · ${g.behavior.unique_destinations} destinations</span></div>` +
    `<div class="grid-main">${panel("Observed relationships", graphSvg(g) + `<div class="table-footer"><span>Click an address to focus the investigation.</span><span>${g.corroborated_pairs.length} cross-source pair${g.corroborated_pairs.length === 1 ? "" : "s"}</span></div>`, badge("Computed from events", "green"))}${panel(
      "Detection signals",
      `<div class="panel-body">${
        alerts
          ...
          .map(
            (a) =>
              `<div class="source-row"><div><button class="row-button" data-action="event" data-id="${a.event_id}">${esc(a.title)}</button><small>${esc(a.source)} · ${esc(a.mode)}</small></div><div class="right">${badge(a.severity)}</div></div>`,
          )
          .join("") ||
        '<p class="tiny muted">No configured detection rule has matched.</p>'
      }</div>`,
      badge(alerts.length + " recent", "amber"),
    )}</div>` +
    notice(
  ...
  switch (a) {
    ...
    case "do-export": {
      ...
      await download(
        ...
        "aegis-" + format + (format === "csv" ? ".csv" : ".jsonl"),
      );
      closeModal();
      return;
    }
    ...
    case "add-sink":
      modal(
        ...
        `<div class="field"><label for="sink-name">Output name</label><input id="sink-name" placeholder="Local SIEM collector"></div><div class="field"><label for="sink-url">HTTP(S) endpoint</label><input id="sink-url" placeholder="http://127.0.0.1:9000/events"></div>${notice("The endpoint must accept POST requests containing an AEGIS event envelope. The delivery ID is sent as an Idempotency-Key. Delivery retries are at least once; the receiver should deduplicate.")}`,
        button("Cancel", "close") +
...
$("#login-form").addEventListener("submit", async (e) => {
  ...
  try {
    await api("/status");
    sessionStorage.setItem("aegis-token", S.token);
    $("#login").hidden = true;
    $("#access-token").value = "";
    await refresh(true);
  } catch (err) {
