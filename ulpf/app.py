"""AEGIS local API and dashboard. No cloud services or CDN assets required."""
import asyncio
import base64
import csv
...
def create_app(data_dir=None, start_receivers=True):
    directory=Path(data_dir or os.getenv("AEGIS_DATA_DIR","data"))
    ...
    if len(token)<16: raise ValueError("AEGIS_ADMIN_TOKEN must contain at least 16 characters")
    ...
    async def lifespan(app):
        ...
        receiver=Receivers(store,os.getenv("AEGIS_SYSLOG_HOST","127.0.0.1"),int(os.getenv("AEGIS_SYSLOG_PORT","5514")),
                           int(os.getenv("AEGIS_TLS_PORT","6514")),os.getenv("AEGIS_TLS_CERT"),os.getenv("AEGIS_TLS_KEY"),os.getenv("AEGIS_TLS_CA"))
        app.state.receivers=receiver
        lab_receiver=Receivers(store,"127.0.0.1",int(os.getenv("AEGIS_LAB_PORT","5515")),mode="lab")
        ...
        print(f"AEGIS ready. Admin access token: {token_path.resolve()} (or AEGIS_ADMIN_TOKEN).",flush=True)
        yield
        app.state.stop.set()
    ...
    app=FastAPI(title="AEGIS Universal Log Pre-processing Framework",version="1.0.0",lifespan=lifespan,docs_url=None,redoc_url=None,openapi_url=None)
    ...
    @app.get("/health")
    def health(): return {"service":"aegis","status":"ok"}
    ...
    def metrics():
        ...
        return Response("".join(f"# TYPE aegis_{k} gauge\naegis_{k} {v}\n" for k,v in values.items()),media_type="text/plain")
    @api.get("/stats")
    def stats(): return store().stats()
    @api.get("/schema")
    def schema(): return {"schema":SCHEMA,"hash":SCHEMA_HASH,"conformance":"AEGIS schema using OCSF concepts; not full OCSF certification"}
    @api.get("/sources")
    ...
    def export(format:str="jsonl",q:str="",status:str="",source:str="",mode:str="",offset:int=0):
        ...
        return Response(content,media_type=media,headers={"Content-Disposition":f'attachment; filename="aegis-{format}.{ext}"',"X-Export-Limit":"1000"})
    @api.get("/candidates")
    ...
    def bundle(pid:str):
        ...
        return Response(row["bundle"],media_type="application/json",headers={"Content-Disposition":'attachment; filename="parser.aegis.json"'})
    ...
    @api.get("/graph")
    def graph(ip:str=""): return store().relationships(ip)
    @api.get("/alerts")
...
if __name__=="__main__":
    import uvicorn
    uvicorn.run(create_app(),host=os.getenv("AEGIS_HOST","127.0.0.1"),port=int(os.getenv("AEGIS_PORT","8765")),log_level="info")
