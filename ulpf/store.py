...
class Store:
    ...
    def relationships(self, ip=""):
        rows = self.events(limit=1000,q=ip)["items"]
        ...
        for row in reversed(rows):
            ...
            timeline.append({"id":row["id"],"time":n["time"],"time_basis":n["time_basis"],"source":row["source"],"mode":row["mode"],"source_ip":source,"destination_ip":dest,"port":n.get("destination_port"),"action":n["action"],"message":n["message"],"severity":n["severity"]})
        return {"nodes":list(nodes.values()),"edges":edges,"timeline":sorted(timeline,key=lambda x:x["time"]),
                ...
                "scope":"Latest 1,000 matching events; address co-occurrence is a relationship, not proof of attack or identity."}
