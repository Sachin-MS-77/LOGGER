...
SCHEMA = {
    "name": "aegis.network_event", "version": "1.0.0",
...
def ecs_export(event):
    """A conservative ECS field mapping, preserving the complete AEGIS event."""
    ...
    return {"@timestamp": n.get("time", event["received_at"]), "ecs": {"version": "8.11.0"},
            ...
            "aegis": {"event": n, "status": event["status"], "raw_reference": f"/api/events/{event['id']}/raw"}}
