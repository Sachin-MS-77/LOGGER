...
For a wider worker trial, set `LOGFLUX_REDPANDA_SMP=4`, then run 16 partitions and 16 workers. The broker gateway acceptance path separately sustained 15,000 durably verified events/s for 60 seconds; see [benchmark-15k.json](benchmark-15k.json). These settings increase disposable lab containers only and do not alter the single-node dashboard. Replicated production HA still requires separately provisioned broker/columnar nodes.
