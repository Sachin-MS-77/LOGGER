...
For a wider local scale trial, set `LOGFLUX_REDPANDA_SMP=4`, then run 16 partitions and 16 workers. These settings increase the disposable lab containers only; they do not alter the single-node dashboard. On the development machine, a 20,000-event 16×16 run reached 6,597.70 worker-path events/s with all hashes, proofs and resume checks passing. Reaching 15,000 requires more than partition count on this host: use separately provisioned broker/columnar nodes and repeat the same verification.
